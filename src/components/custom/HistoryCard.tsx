...
function HistoryCard({ className = "" }: HistoryCardProps) {
  ...
  return (
    <div className={`p-6 shadow-lg flex-1 min-w-[320px] ${className}`}>
      ...
      <div className="space-y-3 max-h-80 overflow-y-auto">
        {logs.length === 0 && !loading ? (
          <div className="text-center py-8">
            <CgFileDocument className="w-8 h-8 text-white flex items-center justify-center mx-auto mb-3" />
            <p className="text-white text-sm">No hay parcelas eliminadas</p>
          </div>
        ) : (
          logs.map((log) => (
            <div
